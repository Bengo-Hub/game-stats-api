...
func (_q *EventQuery) loadManagedBy(ctx context.Context, query *UserQuery, nodes []*Event, init func(*Event), assign func(*Event, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(event.ManagedByTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(event.ManagedByPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(event.ManagedByPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(event.ManagedByPrimaryKey[0]))
