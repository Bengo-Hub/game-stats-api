...
func (_q *LocationQuery) QueryCountry() *CountryQuery {
...
func (_q *LocationQuery) QueryFields() *FieldQuery {
...
func (_q *LocationQuery) QueryEvents() *EventQuery {
...
func (_q *LocationQuery) QueryTeams() *TeamQuery {
...
func (_q *LocationQuery) WithCountry(opts ...func(*CountryQuery)) *LocationQuery {
	query := (&CountryClient{config: _q.config}).Query()
...
func (_q *LocationQuery) WithFields(opts ...func(*FieldQuery)) *LocationQuery {
	query := (&FieldClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *LocationQuery) WithEvents(opts ...func(*EventQuery)) *LocationQuery {
	query := (&EventClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *LocationQuery) WithTeams(opts ...func(*TeamQuery)) *LocationQuery {
	query := (&TeamClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *LocationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Location, error) {
	...
	if query := _q.withFields; query != nil {
		if err := _q.loadFields(ctx, query, nodes,
			func(n *Location) { n.Edges.Fields = []*Field{} },
			func(n *Location, e *Field) { n.Edges.Fields = append(n.Edges.Fields, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withEvents; query != nil {
		if err := _q.loadEvents(ctx, query, nodes,
			func(n *Location) { n.Edges.Events = []*Event{} },
			func(n *Location, e *Event) { n.Edges.Events = append(n.Edges.Events, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withTeams; query != nil {
		if err := _q.loadTeams(ctx, query, nodes,
			func(n *Location) { n.Edges.Teams = []*Team{} },
			func(n *Location, e *Team) { n.Edges.Teams = append(n.Edges.Teams, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *LocationQuery) loadCountry(ctx context.Context, query *CountryQuery, nodes []*Location, init func(*Location), assign func(*Location, *Country)) error {
...
func (_q *LocationQuery) loadFields(ctx context.Context, query *FieldQuery, nodes []*Location, init func(*Location), assign func(*Location, *Field)) error {
	...
	query.withFKs = true
	query.Where(predicate.Field(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(location.FieldsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.location_fields
		if fk == nil {
			return fmt.Errorf(`foreign-key "location_fields" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "location_fields" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *LocationQuery) loadEvents(ctx context.Context, query *EventQuery, nodes []*Location, init func(*Location), assign func(*Location, *Event)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Location)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Event(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(location.EventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.location_events
		if fk == nil {
			return fmt.Errorf(`foreign-key "location_events" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "location_events" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *LocationQuery) loadTeams(ctx context.Context, query *TeamQuery, nodes []*Location, init func(*Location), assign func(*Location, *Team)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Location)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
