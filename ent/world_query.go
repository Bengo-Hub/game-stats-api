...
// QueryContinents chains the current query on the "continents" edge.
func (_q *WorldQuery) QueryContinents() *ContinentQuery {
...
func (_q *WorldQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (_q *WorldQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (_q *WorldQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (_q *WorldQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (_q *WorldQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (_q *WorldQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *WorldQuery) Clone() *WorldQuery {
	...
	return &WorldQuery{
		...
		withContinents: _q.withContinents.Clone(),
...
// WithContinents tells the query-builder to eager-load the nodes that are connected to
// the "continents" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *WorldQuery) WithContinents(opts ...func(*ContinentQuery)) *WorldQuery {
	query := (&ContinentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withContinents = query
	return _q
}
...
func (_q *WorldQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*World, error) {
	...
	if query := _q.withContinents; query != nil {
		if err := _q.loadContinents(ctx, query, nodes,
			func(n *World) { n.Edges.Continents = []*Continent{} },
			func(n *World, e *Continent) { n.Edges.Continents = append(n.Edges.Continents, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *WorldQuery) loadContinents(ctx context.Context, query *ContinentQuery, nodes []*World, init func(*World), assign func(*World, *Continent)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*World)
	for i := range nodes {
...
func (_q *WorldQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(world.Table, world.Columns, sqlgraph.NewFieldSpec(world.FieldID, field.TypeUUID))
	_spec.From = _q.sql
