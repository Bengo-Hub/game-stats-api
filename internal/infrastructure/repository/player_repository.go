...
func (r *playerRepository) Create(ctx context.Context, p *ent.Player) (*ent.Player, error) {
	builder := r.client.Player.Create().
		SetName(p.Name).
	...
	if len(p.Edges.Teams) > 0 {
		ids := make([]uuid.UUID, len(p.Edges.Teams))
		for i, t := range p.Edges.Teams {
			ids[i] = t.ID
		}
		builder.AddTeamIDs(ids...)
	}
...
func (r *playerRepository) Update(ctx context.Context, p *ent.Player) (*ent.Player, error) {
	updater := r.client.Player.UpdateOneID(p.ID).
		SetName(p.Name).
		SetNillableEmail(p.Email).
		SetGender(p.Gender).
		SetNillableDateOfBirth(p.DateOfBirth).
